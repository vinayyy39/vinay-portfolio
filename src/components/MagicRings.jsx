...
// ... (Keep your vertexShader and fragmentShader exactly the same as before) ...
const vertexShader = `
void main() {
...
export default function MagicRings({
  color = '#A855F7',
  colorTwo = '#6366F1',
  speed = 0.5, // Slower speed looks better for background
  ringCount = 8, // More rings fill the screen better
  attenuation = 8,
  lineThickness = 1.5,
  baseRadius = 0.2,
  radiusStep = 0.08,
  scaleRate = 0.15,
  opacity = 0.6, // Lower opacity so text is readable
  blur = 2, // Slight blur makes it look more like a background
  noiseAmount = 0.05,
  rotation = 0,
  ringGap = 1.5,
  fadeIn = 0.5,
  fadeOut = 0.5,
  followMouse = true, // Enable mouse interaction
  mouseInfluence = 0.1,
  hoverScale = 1.1,
  parallax = 0.02,
  clickBurst = false,
  ...
  useEffect(() => {
    ...
    // Add global listeners since it's a full background
    ...
    const animate = (t) => {
      ...
      smoothMouseRef.current[0] += (mouseRef.current[0] - smoothMouseRef.current[0]) * 0.05;
      smoothMouseRef.current[1] += (mouseRef.current[1] - smoothMouseRef.current[1]) * 0.05;
  ...
  // Note: We removed the inline style for blur here to handle it in CSS for performance
  return <div ref={mountRef} className="magic-rings-bg" />;
}